...
make_reference_plot <- function(pair_values, show_legend = TRUE) {
    ...
    p <- ggplot(summary_long, aes(y = Proportion, color = Metric, fill = Metric, shape = Reference)) +
        ...
        geom_text(aes(x = as.numeric(Reference), label = scales::percent(Proportion, accuracy = 0.1)),
...
target_genes_map <- c("INSL3" = "P51460", "SPINT3" = "P49223", "LEP" = "P41159")
representative_values_all <- long_df %>%
    filter(Batch %in% selected_batches, Batch != "SOM_P2_B2", UniqueID %in% unname(target_genes_map), Sample %in% c("M", "Y", "P", "X", "F"),
...
p_rbb <- ggplot(rbb_values, aes(RBB, fill = Batch, color = Batch)) +
    ...
    labs(x = "Relative blank burden", y = "Density of features") + plasmix_theme +
...
bottom_row <- ggarrange(p_rbb, p_blank_srr, nrow = 1, widths = c(1, 1.35), labels = c("c", "d"),
                                label.x = 0, label.y = 1, hjust = -0.2, vjust = 1.2, font.label = label_style)
figure_ed8 <- ggarrange(top_row, bottom_row, ncol = 1, heights = c(1, 1))
...
ggsave(figure_pdf, figure_ed8, width = 10, height = 6.6)
ggsave(figure_png, figure_ed8, width = 10, height = 6.6, dpi = 600, bg = "white")
