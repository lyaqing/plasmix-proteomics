# Extended Data Figure 2 | SomaScan reshaping effects

# 0. Setup ----
source("scripts/_project_setup.R")
use_packages(c("data.table", "tidyverse", "readxl", "ggpubr", "openxlsx", "showtext"), "SomaDataIO")
...
paths <- c(trc = "results/trc_feature_level.tsv.gz", dea = "results/dea_df_multi.tsv.gz", feature_metadata = "data/feature_metadata.tsv.gz",
...
trc_feature_level <- fread(paths[["trc"]])
dea_df_multi <- fread(paths[["dea"]])
feature_metadata <- fread(paths[["feature_metadata"]]) %>% filter(Platform == "SOM", !Is_Protein_Group, !Is_Unknown) %>% select(AssayID, UniqueID) %>% distinct()
anno_11k_file <- paths[["annotation_11k"]]
anno_9k_file <- paths[["annotation_9k"]]
...
anno_11k <- SomaDataIO::read_annotations(anno_11k_file) %>% filter(Organism == "Human") %>%
...
anno_9k <- read_excel(anno_9k_file, range = "B7:I10464") %>% filter(Organism == "Human") %>%
...
annotation_choice <- trc_som %>% distinct(Batch, UniqueID) %>% inner_join(anno_combined, by = "UniqueID") %>%
    count(Batch, Annotation_panel, name = "Matched_features") %>% arrange(Batch, desc(Matched_features), Annotation_panel) %>%
    group_by(Batch) %>% slice(1) %>% ungroup()
print(annotation_choice)
...
df_transition_feature <- trc_som %>% inner_join(batch_annotation, by = c("Batch", "UniqueID")) %>%
    mutate(Batch = factor(Batch, levels = target_batches), Dilution_Bin = factor(Dilution_Bin, levels = c("High conc", "Med conc", "Low conc")),
...
p_transition <- ggplot(df_transition_summary, aes(x = Batch, y = Count, fill = Response_Transition)) +
    ...
    labs(x = NULL, y = "Percentage of features (%)", fill = NULL) +
    plasmix_theme +
...
generate_cat_plot <- function(target_pair) {
    ...
    cat_plot <- ggplot(cat_data, aes(x = TopN, y = Consistency, color = Batch)) +
        ...
        scale_x_log10(breaks = c(10, 100, 1000, 10000), expand = c(0, 0)) +
        scale_y_continuous(limits = c(70, 100), breaks = seq(70, 100, 10), expand = c(0, 0)) +
        labs(x = expression(Top~italic(k)~features), y = paste("Concordance of", target_pair, "(%)")) +
        plasmix_theme +
        theme(legend.margin = margin(-2, 5, 5, 5), legend.title = element_text(size = 7.5, face = "bold", vjust = 0.5),
              ...
              axis.text.x = element_text(hjust = c(0.1, 0.5, 0.5, 0.9)), plot.title = element_text(face = "bold", size = 11, hjust = 0.5)) +
...
pval_facet <- ggplot(df_pval_all, aes(x = -log10(P_Pri), y = -log10(P_Out))) +
    ...
    labs(x = expression("-log"[10] * "P (Calibrated)"), y = expression("-log"[10] * "P (Reshaped)")) +
    plasmix_theme +
...
ggsave("figures/ed2_som_reshaping.pdf", fig_supp_final, width = 10, height = 6)
ggsave("figures/ed2_som_reshaping.png", fig_supp_final, width = 10, height = 6, dpi = 600, bg = "white")
...
write.xlsx(output_list, "tables/SourceData_EDFigure2.xlsx", overwrite = TRUE, keepNA = TRUE, na.string = "NA")
